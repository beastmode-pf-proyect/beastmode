...
const UserRoutines = () => {
  ...
  const fetchAllUserRoutines = async () => {
    ...
    try {
      ...
      const grouped = (data || []).reduce((acc: GroupedRoutines, routine) => {
        const userId = routine.user.id;
        if (!acc[userId]) {
          acc[userId] = [];
        }
        acc[userId].push(routine);
        return acc;
      }, {});
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h2 className="text-2xl font-bold text-gray-800 mb-6">Usuarios con Rutinas</h2>
            ...
            <div className="p-6">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-xl font-bold text-gray-800">
                  Rutinas de {userRoutines[selectedUser][0].user.name || userRoutines[selectedUser][0].user.email}
                </h3>
                <button 
              ...
              <div className="space-y-4">
                {userRoutines[selectedUser].map((routine) => (
                  <div key={routine.id} className="border rounded-lg p-4">
                    <h4 className="font-semibold text-gray-800">{routine.routine.name}</h4>
                    <p className="text-sm text-gray-600 mt-1">{routine.routine.description}</p>
                    <p className="text-xs text-gray-400 mt-2">
                      Asignada: {new Date(routine.created_at).toLocaleDateString()}
                    </p>
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
